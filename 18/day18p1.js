...
const trees = fs.readFileSync('input2.txt')
    .toString()
    .split('\n')
    .map(x => genereate_tree(x, 0)[0])


function genereate_tree(text, idx) {
    if(idx >= text.length) {
    ...
    ++idx
    const left = genereate_tree(text, idx)
    idx = left[1] + 1
    const right = genereate_tree(text, idx)
    idx = right[1] + 1
    node.left = left[0]
    node.right = right[0]
...
for(let i = 1; i < trees.length; ++i) {
    root = add_trees(root, trees[i])
}

console.log(get_magnitude(root))